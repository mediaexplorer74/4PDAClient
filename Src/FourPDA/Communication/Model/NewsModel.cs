// FourPDA.Communication.Model.NewsModel
...
namespace FourPDA.Communication.Model
{
    ...
    {
        public NewsModel(string id, string name, string parentId)
        {
            this.Id = id;
            this.Name = name;
            this.ParentId = parentId;
            this.Children = new List<NewsModel>();
        }

        public NewsModel GetChild(string forumId)
        {
            return this.Id == forumId
                      ? this
                      : Enumerable.FirstOrDefault<NewsModel>(Enumerable.Select<NewsModel, NewsModel>(
                          (IEnumerable<NewsModel>)this.Children, (Func<NewsModel, NewsModel>)
                          (c => c.GetChild(forumId))), (Func<NewsModel, bool>)(f => f != null));
        }

        public IEnumerable<NewsModel> AllChildren
        {
            get
            {
                yield return this;
                foreach (NewsModel child in this.Children)
                {
                    foreach (NewsModel iteratedChild in child.AllChildren)
                        yield return iteratedChild;
                }
            }
        }

        public List<NewsModel> Children { get; private set; }
        ...
        public override string ToString()
        {
            ...
            IEnumerable<NewsModel> allChildren = this.AllChildren;
            bool flag1 = false;
            string str1;
            if (allChildren != null)
            {
            ...
            List<NewsModel> children = this.Children;
            bool flag2 = false;
            string str3;
            if (children != null)
            {
                ...
                while (enumerator.MoveNext())
                {
                    ...
                    string str4;
                    if (enumerator.Current != null)
                        str4 = string.Format((IFormatProvider)CultureInfo.InvariantCulture, 
                            "{0}", new object[1]
                        {
              enumerator.Current
                        });
